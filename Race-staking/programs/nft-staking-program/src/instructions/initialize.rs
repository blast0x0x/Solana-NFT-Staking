...
pub struct InitializeNFT<'info> {
    ...
    #[account(
        init,
        seeds = [RS_NFT_SEED.as_ref(), nft_mint.key().as_ref()],
        bump,
        payer = admin,
        space = 8 + std::mem::size_of::<NftInfo>(),
    )]
...
pub fn initialize_nft(ctx: Context<InitializeNFT>, class_id: u16) -> Result<()> {
    msg!("initialize_nft");

    let nft_info = &mut ctx.accounts.nft_info;
    nft_info.class_id = class_id as u32;
    nft_info.nft_addr = ctx.accounts.nft_mint.key();
    Ok(())
}
