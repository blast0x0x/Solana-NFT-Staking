...
export const getTokenAccount = async (mintPk, userPk) => {
...
export const getNFTTokenAccount = async (nftMintPk) => {
...
export const getOwnerOfNFT = async (nftMintPk) => {
  let provider = await getProvider();
...
export const getAllNftData = async () => {
  try {
    ...
    const result = isValidSolanaAddress(pubKey);
    if (result) {
      const nfts = await getParsedNftAccountsByOwner({
        publicAddress: pubKey.toBase58(),
...
export const getNftMetadataURI = async (nftMintPk) => {
  try {
    let provider = await getProvider();
